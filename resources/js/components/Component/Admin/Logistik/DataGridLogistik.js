...
import {getLogistic,deleteShipment} from '../../../Redux/Admin/action'
...
const headCells = [
  ...
  { id: 'receipt_no', numeric: true, disablePadding: false, label: 'Nomor Resi' },
  { id: 'receipt_link', numeric: true, disablePadding: false, label: 'Link Resi' },
      ...
      <Tooltip title="Delete" style={numSelected > 0 ?{}:{display:"none"}}>
          <IconButton aria-label="delete"
            onClick={async()=>{
              ...
              const resp = await dispatch(deleteShipment(payload));
              if(resp.type === 'DELETE_SHIPMENT_ADMIN_SUCCESS'){
...
export default function TableLogistic() {
  ...
  const [searched, setSearched] = useState();
  const rows = useSelector((state) => state.admin.logistics.data);
