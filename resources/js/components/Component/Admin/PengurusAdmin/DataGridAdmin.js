...
import {deleteAdminMember, getAdminMember} from '../../../Redux/Admin/action'
...
const headCells = [
  { id: 'id', numeric: false, disablePadding: true, label: 'ID Member' },
    ...
    >
      ...
      <Tooltip title="Delete" style={numSelected > 0 ?{}:{display:"none"}}>
          ...
          onClick={async()=>{
              ...
              //console.log(payload)
              const resp = await dispatch(deleteAdminMember(payload));
              if(resp.type === 'DELETE_MEMBER_ADMIN_SUCCESS'){
...
export default function TabelAdminMember() {
  ...
  const [searched, setSearched] = useState();
  const rows = useSelector((state) => state.admin.admins.data);
  const [data ,setRows] = React.useState(rows);
  
  useEffect(() => {
    dispatch(getAdminMember())
  }, []);
  const requestSearch = (searchedVal) => {
    ...
    <div className={classes.root}>
      <div style={{display:"flex"}}>
        ...
        <div style={{
          ...
          <Button className={classes.buttonAdd} onClick={()=> {history.push("/admin/pengguna/tambah")}}>Tambah &nbsp; &nbsp;<BsPersonPlusFill/></Button>
        </div>
      </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center"><Button style={{color:"#000",marginRight:5, backgroundColor:'transparent', textTransform:'capitalize'}}
                        onClick={()=>{
                          history.push('/admin/pengguna/edit/'+row.id)
                        }}
